-- ============================================================================
-- Behaviour Intelligence System - Temporal Analysis
-- ============================================================================
-- Adds multi-session context: recency, frequency, trend direction, velocity,
-- and temporal state detection (Returning Evaluator, Re-engaged Prospect,
-- Persistent Hesitation, Chronic Stall).
--
-- Input:  your-project.your_dataset.classified_sessions
--         (output of 02-state-classification.sql)
-- Output: One row per user with temporal context.
-- ============================================================================

WITH classified_sessions AS (
  SELECT * FROM `your-project.your_dataset.classified_sessions`
),
...
trend_analysis AS (
  SELECT
    ...
    CASE
      ...
      -- Reinforcing: both progression and depth non-decreasing across recent sessions.
      -- Matches JS calculateTrend() which requires >= 2 sessions (not 3).
